...
void VideoPlayer::SetInfo(std::string _Name, int _StartFrame, int _EndFrame, float _Time)
{
	...
	for (int Frame = StartFrame_; Frame <= EndFrame_; Frame++)
	{
		...
		_VideoFrameRenderers.push_back(VideoFrameRenderer);
	}
}
...
void VideoPlayer::Update()
{
	...
	if (CurrentFrame < _VideoFrameRenderers.size())
	{
		for (GameEngineRenderer* VideoFrameRenderer : _VideoFrameRenderers)
		{
			VideoFrameRenderer->SetScale(float4(0, 0));
		}

		_VideoFrameRenderers[CurrentFrame]->SetImageScale();
	}
	else
	{
		VideoFinished_ = true;
